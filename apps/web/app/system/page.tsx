...
export default async function SystemPage() {
  try {
    ...
    return <section className="mx-auto max-w-[1100px] px-6 py-7"><h1 className="mb-4 text-2xl font-semibold">System</h1><div className="grid gap-4 md:grid-cols-2"><Panel title="TraceForge" status={health.overall_status}><p className="text-forge-muted">Product operational status</p></Panel><Panel title="Storage" status={health.storage.status}><p className="text-forge-muted">PostgreSQL connectivity</p></Panel><Panel title="Ingestion" status={health.ingestion.status}><p className="text-forge-muted">Last telemetry received</p><code className="mt-1 block text-sm">{health.ingestion.last_telemetry_received_at ? formatDateTime(health.ingestion.last_telemetry_received_at) : "—"}</code></Panel><Panel title="Analysis" status={health.analysis.status}><dl className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm"><dt className="text-forge-muted">Pending jobs</dt><dd>{health.analysis.pending_jobs}</dd><dt className="text-forge-muted">Running jobs</dt><dd>{health.analysis.running_jobs}</dd><dt className="text-forge-muted">Recent failed jobs</dt><dd>{health.analysis.failed_jobs_recent}</dd><dt className="text-forge-muted">Oldest pending age</dt><dd>{age(health.analysis.oldest_pending_job_age_ms)}</dd></dl></Panel></div></section>;
  } catch {
